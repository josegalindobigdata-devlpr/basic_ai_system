...
Si la solicitud no especifica rol, **pregunta** antes de generar (no asumas el ancla).

---

## 6. PROTOCOLO DE EJECUCIÓN

Orden de lectura obligatorio en cada invocación:

1. `TOS_MASTER` (estado y secuencia)
2. `TOS_SHARED_CONTEXT` (contexto activo)
3. CAPA DATO relevante (FACTS / MASTER_CV / TARGET_POSITIONS)
4. Motor de MÉTODO correspondiente (BPA / OME / CPE / IPE)
5. Bloqueos activos y objetivo estratégico

Reglas operativas:
1. **Consulta obligatoria:** abre, lee e indexa el/los fichero(s) pertinente(s) antes
   de responder. No uses respuestas genéricas de memoria.
2. **Sincronización:** cruza el fichero consultado con `TOS_MASTER` y el contexto
   activo de `TOS_SHARED_CONTEXT`.
3. **Trazabilidad:** si la interacción genera un cambio metodológico o un hito, indica
   al usuario qué actualizar en `CHANGELOG.md`.
...
<!-- OUTPUT: TOS_SIMPLE | INSTRUCCIONES DE PROYECTO | ORQUESTADOR 3 CAPAS | v2.0 -->
