# FACT_SHEETS — Datos invariables del perfil : FACT_SHEETS.md
<!-- v3.1 — Derivado de MASTER_CV_RAW.md v3.0 (auditado y consolidado) -->
<!-- Regla de uso: todo dato de este documento se cita LITERALMENTE en cualquier entregable (CV, LinkedIn, cartas). No se reformula. -->
<!-- #¿Que Gobierna? Cómo citar datos invariantes. #Reglas: Naming, horas, fechas de formación, periodos consolidados. -->
<!-- # Release_base_profile: v.3.0 -->
<!-- TOS_SIMPLE v4.0 — Arquitectura de 4 capas (+ CAPA ENTREGABLE). Sustituye a v3.x. -->

---

## 1. Identidad profesional

- **Nombre completo (oficial):** José Francisco Galindo Egea
  - Uso: cabecera de los CV Ancla y nombre de perfil en LinkedIn
- **Nombre comercial / marca personal:** Jose Galindo
  - Uso: dominio propio, portfolio, firma corta, comunicación informal
- **Dominio propio:** josegalindo.net
- **Ubicación:** Madrid, España
- **Movilidad / disponibilidad:** "Disponibilidad inmediata. Movilidad nacional/internacional: sí. Remoto/híbrido"
- **Email profesional (candidaturas):** jose@josegalindo.net
  - Uso: CV Ancla, LinkedIn, cover letters y comunicación directa de candidaturas
- **Email secundario:** contact@josegalindo.net
  - Uso: web/portfolio, formularios y temas generales
- **Email freelance:** opportunities@josegalindo.net
  - Uso: propuestas de colaboración y servicios freelance (rol Senior Transformation & Delivery Advisor)
- **Teléfono:** +34 682 832 341
- **URL LinkedIn:** (https://www.linkedin.com/in/josegalindo/)

**Regla de uso del nombre en entregables:**
- CV Ancla y LinkedIn → **José Francisco Galindo Egea** (nombre completo)
- Portfolio, web, firma breve y materiales de marca → **Jose Galindo**
- Cover letters → nombre completo en firma; "Jose Galindo" admisible en cuerpo si el tono lo permite

---

## 2. Titulares oficiales por rol ancla
<!-- Titulares VALIDADOS (inmutables, salvo adaptación por oferta). CV: versión limpia sin métrica de horas (la escala vive en Perfil/Logros). LinkedIn: variante con métrica. Adaptación por oferta: ajustar el título a la oferta y las palancas a sus keywords. -->

**DECISIÓN DOCUMENTADA (v4.0) — Idioma del titular en CV Ancla:** en los CV Ancla, el
TITULAR de cabecera se cita en **inglés** incluso en la versión ES (se usa la variante
`CV — EN`), por convención de mercado TIC y compatibilidad ATS. La variante `CV — ES` se
conserva como respaldo para ofertas que exijan explícitamente el título en español. El
**cuerpo** del CV ES permanece en español. Esto resuelve la aparente discrepancia
"`CV_ANCLA_*_ESP` con titular en inglés": es comportamiento **canónico**, no desviación.


### GOV — Governance / PMO
- **CV — ES:** IT Governance & PMO Lead | Gobierno de Portfolios, Control Financiero y Reporting Ejecutivo | Banca y Seguros
- **CV — EN:** IT Governance & PMO Lead | Portfolio Governance, Financial Control & Executive Reporting | Banking & Insurance
- **LinkedIn (EN, con métrica):** IT Governance & PMO Lead | Enterprise Portfolio Governance 200K+ hrs · Financial Control · SteerCo Reporting | Banking & Insurance

### SDM — Service Delivery / Managed Services
- **CV — ES:** Senior IT Service Delivery Manager | Servicios Gestionados, SLA y P&L | Banca, Seguros y Multiproveedor
- **CV — EN:** Senior IT Service Delivery Manager | Managed Services, SLA & P&L Governance | Banking, Insurance & Multi-vendor
- **LinkedIn (EN, con métrica):** Senior IT Service Delivery Manager | Managed Services 130K+ hrs/yr · SLA & P&L · Multi-vendor | Banking & Insurance

### DLV — Delivery (técnico)
- **CV — ES:** Senior IT Delivery Manager | Plataformas de Datos, SDLC y Delivery Regulatorio | Banca, Seguros y Telecom
- **CV — EN:** Senior IT Delivery Manager | Data Platforms, SDLC & Regulatory Delivery | Banking, Insurance & Telecom
- **LinkedIn (EN, con métrica):** Senior IT Delivery Manager | Data Platforms (DWH/Big Data) · SDLC & Release · Regulatory Reporting | Banking & Insurance

### LinkedIn — perfil paraguas (headline único, el que está vivo)
Senior IT Service Delivery & Governance Leader | PMO, Operational Excellence & Enterprise Transformation | Banking, Insurance & Regulated Environments
<!-- Las variantes "LinkedIn (con métrica)" por ancla de arriba NO son el headline principal (que es único y paraguas): se usan para campañas dirigidas, mensajes de outreach o el bloque destacado. -->
---

## 3. Certificaciones
<!-- Citar siempre con nombre exacto, entidad y fechas. -->

| Certificación | Entidad | Obtención | Vigencia | Credential ID |
|---|---|---|---|---|
| PMP® — Project Management Professional | PMI | 03/2026 | 03/2029 | 4323923 |
| ITIL® 4 Foundation | PeopleCert | 12/2025 | Sin caducidad si Foundation | GR671849348JG

**Marcos y estándares de conocimiento aplicado (no certificados):**
COBIT, CMMI, ISO 12207, ISO 15504, ISO 20000, ISO 21500, ISO 31000

---

## 4. Formación
<!-- Forma exacta de citar cada titulación. -->

**REGLA DE CITACIÓN DE FORMACIÓN (anti-edadismo con excepciones operativas):**
- **Principio base:** minimizar la información temporal de las titulaciones para no introducir señales de edad innecesarias. No se citan intervalos ni año de inicio salvo donde la plataforma o el proceso lo exijan.
- **En CV ancla:** se cita ÚNICAMENTE el AÑO DE FINALIZACIÓN de cada titulación (sin intervalo ni año de inicio). Motivo: en proceso siempre se pregunta por la formación; el año de cierre responde a esa pregunta sin exponer el recorrido completo.
- **En LinkedIn:** se cita el INTERVALO REAL (inicio–fin), porque la plataforma lo requiere. Se usan las fechas reales registradas en el MASTER, sin maquillar.
- **Registro interno (FACTS/MASTER):** se conservan los intervalos completos como respaldo; no se citan literalmente en entregables.

- **Ingeniero en Informática** — Universidad de Murcia (UMU) [interno: 1991–1998 · CV: "1998" · LinkedIn: 1991–1998]
- **Executive MBA** — Escuela de Organización Industrial (EOI) [interno: 2010–2011 · CV: "2011" · LinkedIn: 2010–2011]
- **Postgrado en Dirección de Empresas de Telecomunicación** — UPM [interno: 1998–2000 · CV: no se incluye (formación comprimida) · LinkedIn: como nota]

**Versión comprimida canónica para CV:** Executive MBA (2011, EOI) | Ingeniero en Informática (1998, UMU) | PMP® | ITIL®
---

## 5. Idiomas

- **Español:** nativo
- **Inglés:** B2 — experiencia de trabajo en equipos internacionales (UK, USA, Alemania, México)

---

## 6. Cifras de cabecera validadas
<!-- Únicas cifras globales autorizadas para titulares, resúmenes y elevator pitch. Trazables al MASTER y a sus Decisiones Documentadas. -->

- **Años de experiencia (DECISIÓN FIJADA):** "+20 años" — anclado a experiencia en gestión (gestor desde 03/2003; carrera TIC desde 12/1998)
  - Formulación canónica ES: "+20 años liderando servicios, programas y gobierno TIC"
  - Formulación canónica EN: "20+ years leading IT services, programs and governance"
  - Nota: revisar al alza ("+25") cuando se cumplan 25 años de gestión (03/2028)
- **Gobierno PMO de portfolios — PROGRAMA INDIVIDUAL:** más de 200.000 horas de delivery (215.000 h en el mayor programa, Plataforma Informacional BBVA)
  - EN: "single-program PMO governance exceeding 200K delivery hours (215K in the largest)"
- **Gobierno PMO de portfolios — ACUMULADO DE CARRERA:** más de 500.000 horas de delivery gobernadas
  - EN: "cumulative PMO portfolio governance exceeding 500K delivery hours across enterprise programs"
  - Trazabilidad: BBVA Plataforma Informacional 215.000 h (MASTER 1.19) + BBVA Bancomer / México 150.000 h (1.20) + BMG Sistemas Estructurales >190.000 h/año (1.15). Refuerzan el margen, sin contabilizarse para alcanzar el umbral: DWH-RC 82.000 h (1.12), RDA-BI/BCBS 239 58.000 h (1.10), Plan de Sistemas Caixa Geral 90.000 h (1.18).
  - **REGLA DE USO (anti-inflación):** las dos cifras se presentan SIEMPRE diferenciadas — "200K+ en un solo programa / 500K+ acumulado como PMO". Nunca se cita "500K" sin la aclaración "acumulado".
- **Gestión directa de servicios TI:** más de 130.000 horas/año (escala de la cuenta de servicios gestionados insignia actual)
  - EN: "managed services environment above 130K hours/year (current flagship account)"
- **Sectores:** Banca, Seguros, Telecomunicaciones, Energía, Retail, Sector Público
- **Cuentas/clientes de referencia:** Santander, BBVA, Mapfre, Banc Sabadell, Telefónica, Repsol, TGS, Liberbank
- **Programa regulatorio insignia (delivery directo end-to-end):** RDA-BI/BCBS 239 (Santander Consumer Finance) — presentar como "58.000 horas" según Decisión Documentada
- **Mayor cuenta gestionada (P&L):** 6,5 M€/año, 75 profesionales (Santander/Ayesa)

---

## 7. Reglas de citación derivadas de Decisiones Documentadas (MASTER)
<!-- Recordatorios operativos para coherencia entre entregables. El detalle completo vive en el MASTER. -->

1. Bloque Ibermática Digital consolidado en CVs: **02/2020–01/2021, 10.000 h/año**.
2. TGS en CV ancla: **7.000 h/año** (HCS 4.500 + automatización 2.500).
3. RDA-BI en CV ancla: **58.000 horas** (sin tasa anual).
4. Compañía **Ayesa** como bloque en LinkedIn, cerrando la descripción con la nota: "Note: Ibermática — integrated into Ayesa after its acquisition (Dec 2022)." (documenta la transición Ibermática → Ayesa, 29/12/2022).
5. Etapa GFI 12/1998–03/2003: **resumida** en CVs.
6. Los CVs usan el **título unificado ejecutivo** indicado en el campo Rol de cada bloque del MASTER; ese campo tiene prioridad.
7. **Experiencia de cabecera: "+20 años"** anclado a gestión (desde 03/2003). Titulaciones: en CV solo el año de finalización; en LinkedIn el intervalo real (ver §4). La etapa técnica 12/1998–03/2003 se presenta resumida como base técnica, sin alimentar el contador de cabecera.
8. **Transformación (CV ancla):** se cita solo como contexto en los bloques de experiencia (etiqueta del puesto real); nunca en titular, Perfil ni Logros. No se posiciona el perfil como experto en transformación. El perfil paraguas de LinkedIn sí puede presentarla como una de sus tres dimensiones (ver §2). Excepción: adaptación puntual del titular de un CV ancla a una oferta que lo exija explícitamente.
9. **Nombres de empresa — forma, caja y disposición Cliente (Empleador):**
   - **Forma de marca:** marca reconocible (Ayesa, Devoteam, Everis, Atmira),
     no la razón social registral; misma forma en CV, LinkedIn y MASTER.
   - **Caja mixta por defecto:** los nombres se escriben en caja mixta, nunca en
     versalitas/bloque. Excepciones — SIEMPRE en mayúscula:
       · Siglas/acrónimos: GFI, TGS, BBVA, BMG, SCF, CIB.
       · Wordmarks en versalita por decisión documentada: ALSEA, GFI INFORMATICA.
     SIEMPRE en caja mixta (nunca versalita): Devoteam, Mapfre, Kyndryl, Liberbank, Atmira, Repsol.
   - **Disposición Cliente (Empleador):** la línea de empresa de cada bloque se
     compone como **Cliente (Empleador)**, en negrita y caja mixta. El cliente va
     delante porque carga la señal sectorial Tier-1 (Santander, BBVA, Mapfre…);
     el empleador va entre paréntesis como dato contractual y para la detección
     del campo *Company* en ATS. Multi-cliente: "Cliente1 / Cliente2 (Empleador)".
   - **Énfasis único:** un solo recurso fuerte por nivel (negrita en la línea de
     empresa). No se aplica versalita a cliente y empleador a la vez: aplana la
     jerarquía y penaliza la legibilidad.
   - Consolidaciones temporales y bloques propios (p. ej. Santander CIB): ver item 11.
10. **Horas PMO — doble métrica (ver §6):** el acumulado de carrera "500.000+ horas" se cita SIEMPRE diferenciado del programa individual "200.000+ horas". Headline de LinkedIn: se mantiene "200K+" (cifra de programa individual, verificable de un vistazo); el acumulado solo se introduce, si se desea, en el cuerpo del ABOUT con la formulación diferenciada.
11. **Agrupaciones temporales en CV (bloques-compañía):** además de Ibermática (item 1),
    las experiencias se consolidan en CV por:
    - Atmira · PMO Regulatory 09/2013–04/2016 (DWH-RC + DWR-RC + RDA-BI/BCBS 239)
    - Devoteam · Enterprise Transformation 01/2010–08/2012 (BMG + BDP + SIGE)
    - Everis · Plataforma Informacional BBVA 08/2007–07/2009 (BBVA + Bancomer + Caixa Geral)
    - GFI Informática 03/2003–07/2007 (Unidad BI + Proyectos BI)
    El periodo visible en CV es la UNIÓN; los sub-periodos por proyecto viven en MASTER/PORTFOLIO.
    Excepción: Santander CIB (Ibermática) se presenta como bloque propio, no en el clúster TGS/ALSEA.
---
